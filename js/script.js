...
function reScroll(num, e){
    ...
    window.scrollTo(0, sticky.offsetTop + (num * (height / 6) - (height / 6 / 1)))    
}
...
let swiper = new Swiper(".swiper-slider", {
        ...
        on: {
            slideChangeTransitionStart: function() {
                const active = document.querySelector('.features-slider__item.swiper-slide-active'),
                    ...
                    info = document.querySelectorAll('.features-scroller__info div');
                    index = document.querySelector('.features-slider__item.swiper-slide-prev').getAttribute('data-swiper-slide-index');
                    
                featuresReActive(i);
                console.log(images[index], info[index], index)
                // images[index]
    ...
    //             const images = document.querySelectorAll('.features-scroller__images img'),
    //                 info = document.querySelectorAll('.features-scroller__info div'),
    //                 indexPrev = document.querySelector('.features-slider__item.swiper-slide-prev').getAttribute('data-swiper-slide-index'),
    //                 indexNext = document.querySelector('.features-slider__item.swiper-slide-next').getAttribute('data-swiper-slide-index');
                    
    //             featuresReActive(i);

    //             for(let i = 0; i < images.length; i++){
    //                 images[i].classList.remove('prev-img');
    //                 images[i].classList.remove('next-img');
    //                 info[i].classList.remove('prev-text');
    //                 images[i].classList.remove('next-text');
    //             }
    //             images[indexPrev].classList.add('prev-img');
    //             images[indexNext].classList.add('next-img');
    //             info[indexPrev].classList.add('prev-info');
    //             info[indexNext].classList.add('next-info');

    //             console.log(indexPrev, i, indexNext)
    //             // images[index]
